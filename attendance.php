...
if (isset($_POST['employee'])) {
	...
	if ($query->num_rows > 0) {
		...
		$date_now = date('Y-m-d');
		$date_now = date('Y-m-d');
		$date_minus_one_day = date('Y-m-d', strtotime($date_now . ' -1 day'));
		...
		if ($status == 'in') {
			...
			if ($query->num_rows > 0) {
				$output['error'] = true;
				$output['message'] = 'Ya cumpliste hoy';
			} else {
				//updates
			...
			if ($query->num_rows < 1) {
				$output['error'] = true;
				$output['message'] = 'falta hora de salida';
			} else {
				$row = $query->fetch_assoc();
				if ($row['time_out'] != '00:00:00') {
					$output['error'] = true;
					$output['message'] = 'Esta accion ya se realizó hoy';
				} else {
		...
		$output['error'] = true;
		$output['message'] = 'Empleado no encontrado';
	}
}
